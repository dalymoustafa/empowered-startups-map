...
function extractCity(address) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (/^\d+$/.test(part)) continue;
    if (/\b(street|road|avenue|boulevard|floor|level|suite|tower|plaza|bldg|place|district|rua|lote|edificio|salas|hastings|adelino|fonseca)\b/i.test(part)) continue;
...
async function main() {
  ...
  const record = allRecords.find(r =>
    r.fields["Name"] && r.fields["Name"].toLowerCase().includes("empowered startups")
  );

  if (!record) { console.error("Could not find Empowered Startups!"); process.exit(1); }
...
<head>
  ...
  <title>Empowered Startups — Office Locations</title>
